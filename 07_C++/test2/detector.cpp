                            ...
                            const cv::Size& inputSize = cv::Size(640, 640))
{
    ...
    // [수정] for문 중괄호 추가 및 출력 연산자(>>) 오타 수정(<<)
    for (auto shape : inputTensorShape)
    {
        std::cout << "Input shape: " << shape << std::endl;
    }

    // [수정] 최신 ONNX Runtime API (GetInputNameAllocated / GetOutputNameAllocated) 적용
                                                    ...
                                                    const float& confThreshold, const float& iouThreshold)
{
    ...
    // [수정] cv::size -> cv::Size 오타 수정 및 std:; -> std:: 쌍콜론 오타 수정
                                            ...
                                            const float& iouThreshold = 0.45)
{
    ...
    // [수정] std::string 기반 vector를 const char* 포인터 포맷으로 호환 변환
    std::vector<const char*> inputNamesPtr = { inputNames[0].c_str() };
    std::vector<const char*> outputNamesPtr = { outputNames[0].c_str() };
